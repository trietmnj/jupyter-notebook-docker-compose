...
class VQApoisson():
    ...
    :class:`VQApoisson` object works with class: `qiskit.aqua.QuantumInstance` or `backend`.
    ...
    def _calc_Xn(self, params, *, is_shift=False):
        ...
        if self.qinstance.is_statevector:
            sv = np.asarray(self.qinstance.execute(qc).get_statevector(qc))
            val = 0
    ...
    def get_f_vec(self):
        return np.asarray(execute(self.qc_f_vec, Aer.get_backend('statevector_simulator')).result().get_statevector())
    ...
    def get_statevec(self, x):
        ...
        return np.asarray(execute(qc, Aer.get_backend('statevector_simulator')).result().get_statevector())

    
    def get_errors(self, x):

        statevec = self.get_statevec(x) 
        solvec = statevec * self.evaluate(x)[1]

        cl_sol = np.asarray(self.get_cl_sol())
    ...
    def get_sol(self, x):
        return self.get_statevec(x) * self.evaluate(x)[1]
